...
async function showModal(id) {
  ...
  try {
    const response = await fetch(URL + `/get-island/${id}`);
    ...
    cardMap.innerHTML = `<img src="${URL}/img/${id.toLowerCase()}.svg" alt="Pulau ${id}" />`;
...
// Add maps point on each island
const pulauMarkers = {
  Sumatera: { x: 120, y: 140 },
  Jawa: { x: 250, y: 245 },
  Bali: { x: 347, y: 260 },
  Kalimantan: { x: 320, y: 140 },
  Sulawesi: { x: 440, y: 160 },
  Maluku: { x: 570, y: 106 },
  Papua: { x: 730, y: 180 },
};
const svgNS = "http://www.w3.org/2000/svg";

Object.entries(pulauMarkers).forEach(([id, { x, y }]) => {
  const g = document.querySelector(`#${id}`);
  if (!g) return;

  const marker = document.createElementNS(svgNS, "image");
  marker.setAttribute("href", `${URL}/img/maps-point.png`);
...
async function getQuestions(cityName) {
  try {
    const response = await fetch(URL + `/get-questions/${cityName}`);
    const data = await response.json();
